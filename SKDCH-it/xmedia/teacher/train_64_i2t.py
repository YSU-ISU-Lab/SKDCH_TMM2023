...
WORKDIR_2 = '../../dataset/'
DIS_MODEL_PRETRAIN_FILE = '../SCHGAN_I2T/data/I2T_teacher_best_pretrain_' + str(OUTPUT_DIM) + '.model'
DIS_MODEL_BEST_FILE = '../SCHGAN_I2T/data/I2T_teacher_' + str(OUTPUT_DIM) + '.model'
DIS_MODEL_NEWEST_FILE = '../SCHGAN_I2T/data/I2T_teacher_latest_' + str(OUTPUT_DIM) + '.model'
GEN_MODEL_BEST_FILE = '../SCHGAN_I2T/data/I2T_teacher_' + str(OUTPUT_DIM) + '.model'
GEN_MODEL_NEWEST_FILE = '../SCHGAN_I2T/data/I2T_teacher_latest_' + str(OUTPUT_DIM) + '.model'
...
feature_dict = ut.load_all_feature_for_train_2(
    WORKDIR_2)
    ...
    while index < train_size:
        ...
        if index + BATCH_SIZE <= train_size:
            for i in range(index, index + BATCH_SIZE):
                ...
                if query_pair_train[query] == pos:  # 相似矩阵
...
def train_generator(sess, generator, discriminator, train_list, train_pos,
                    flag):
        ...
        for epoch in tqdm.tqdm(range(WHOLE_EPOCH), desc='epoch'):
            for g_epoch in range(G_EPOCH):
                ...
                if (g_epoch + 1) % (G_DISPLAY) == 0:
                    i2t_test_map, t2i_test_map = MAP(sess, generator, test_i2t_pos, test_i2t, test_t2i_pos, test_t2i,
