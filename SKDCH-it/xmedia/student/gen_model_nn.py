...
class GEN:
    def __init__(self, image_dim, text_dim, hidden_dim, output_dim, class_dim, weight_decay, learning_rate, param=None):
        ...
        # 将标签编码成one-hot形式，预训练有正式训练没有，（有监督有无监督没有）
        with tf.variable_scope('generator'):
            if param == None:
                ...
                self.Wq_2 = tf.get_variable('Wq_2', [self.hidden_dim, self.output_dim],
                                            initializer=tf.truncated_normal_initializer(mean=0.0, stddev=0.1))
                self.Wq_3 = tf.get_variable('Wq_3', [self.hidden_dim, self.class_dim],  # class_dim是什么
                                            initializer=tf.truncated_normal_initializer(mean=0.0, stddev=0.1))
        ...
        self.pred_score = -tf.reduce_sum(tf.square(self.text_sig - self.image_sig), 1)
        ...
        self.gen_loss = tf.reduce_mean(tf.abs(self.gen_prob - self.reward)) \
                        + tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=self.text_class_pred, labels=self.text_label)) \
                        + tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=self.image_class_pred, labels=self.image_label)) \
                        + self.weight_decay * (
                                tf.nn.l2_loss(self.Wq_1) + tf.nn.l2_loss(self.Wq_2) + tf.nn.l2_loss(self.Wq_3)
